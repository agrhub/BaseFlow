...
router.get('/:conn/sidebar', async (req, res) => {
    ...
    try {
        const { classes } = await (0, helpers_1.getOrScanRepo)(conn);
...
router.get('/:conn/stats', async (req, res) => {
    const { conn } = req.params;
    try {
        const { repoPath, classes, graph } = await (0, helpers_1.getOrScanRepo)(conn);
            ...
            try {
                ...
                if (devops && devops.provider !== 'none') {
                    ...
                    if (devops.provider === 'github') {
                        remoteCommits = (await (0, helpers_1.fetchFromGitHub)(devops.owner, devops.repo, 'commits?per_page=100', token));
                        commits = remoteCommits.slice(0, 15).map((c) => ({
                        ...
                        remoteCommits = (await (0, helpers_1.fetchFromGitLab)(devops.fullPath, 'repository/commits?per_page=100', token));
                        commits = remoteCommits.slice(0, 15).map((c) => ({
        ...
        try {
            ...
            if (cachedStats) {
                ...
                if (devops && devops.provider !== 'none') {
                    ...
                    const token = devops.profile.options?.gitToken;
                    if (devops.provider === 'github') {
                        statsData = await (0, helpers_1.fetchGitHubRepoStats)(devops.owner, devops.repo, token);
                    }
                    else if (devops.provider === 'gitlab') {
                        statsData = await (0, helpers_1.fetchGitLabRepoStats)(devops.fullPath, token);
                    }
                    if (statsData) {
...
router.get('/:conn/readme-analysis', async (req, res) => {
    ...
    try {
        ...
        const analysis = await (0, helpers_1.generateReadmeAnalysis)(conn, repoPath);
        res.json(analysis);
    }
    catch (error) {
...
router.post('/:conn/class-analysis', async (req, res) => {
    ...
    try {
        const { repoPath, classes } = await (0, helpers_1.getOrScanRepo)(conn);
        ...
        const promptText = (0, prompts_1.getClassAnalysisPrompt)({
            name: targetCls.name,
        ...
        const resultText = await GeminiService_1.geminiService.generateContent(promptText);
...
router.get('/:conn/graph', async (req, res) => {
    const { conn } = req.params;
    try {
        const { graph } = await (0, helpers_1.getOrScanRepo)(conn);
        res.json({
            nodes: graph.nodes,
            edges: graph.edges
        });
...
router.get('/scan-stream', async (req, res) => {
    ...
    try {
        ...
        if (localPath) {
            ...
            targetPath = path.join(helpers_1.tempDir, repoName);
            sendProgress(2, 4, 'Performing ZIP download...');
            if (fs.existsSync(targetPath)) {
                fs.rmSync(targetPath, { recursive: true, force: true });
            }
            await (0, helpers_1.zipCloneOrPull)(repoUrl, targetPath, {});
        ...
        (0, tools_1.setCurrentScanResult)(targetPath, parseResult.classes);
...
router.post('/:conn/sync', async (req, res) => {
    const { conn } = req.params;
    try {
        const cachePath = path.join(helpers_1.tempDir, `${conn}_cache.json`);
        (0, helpers_1.clearRepoCache)(conn);
        const scanData = await (0, helpers_1.getOrScanRepo)(conn);
        ...
        await (0, helpers_1.performCompleteAnalysis)(conn);
        res.json({
            success: true,
...
router.post('/:conn/pull', async (req, res) => {
    ...
    try {
        ...
        (0, helpers_1.clearRepoCache)(conn);
        const cachePath = path.join(helpers_1.tempDir, `${conn}_cache.json`);
        await (0, helpers_1.zipCloneOrPull)(profile.uri, targetPath, profile.options, cachePath);
